  ...
  return <main className="min-h-screen bg-[#0B1020] text-white px-4 py-6 relative">
    ...
    <div className="max-w-md mx-auto">
      <div ref={captureRef} className="text-center mb-7 mt-3 bg-[#0B1020] px-2 py-4 rounded-3xl"><div className="bg-white/5 rounded-3xl p-6">
        ...
        {goalReady && <div className="mb-4"><div className="grid grid-cols-7 gap-1 sm:gap-2">{milestones.map((m) => <button key={m.key} onClick={() => openMilestone(m)} className={`flex flex-col items-center justify-center py-2 rounded-xl transition ${m.claimed ? "bg-emerald-400/10" : m.reached ? "bg-[#7CFF5B]/10 shadow-[0_0_16px_rgba(124,255,91,0.25)] animate-pulse" : "bg-white/5"}`} aria-label={`${m.percent}% milestone`}><span className={`text-base leading-none ${m.reached && !m.claimed ? "scale-110" : ""}`}>{m.claimed ? "✅" : "🎁"}</span><span className={`text-[9px] mt-1 ${m.reached ? "text-gray-200" : "text-gray-500"}`}>{m.percent}%</span></button>)}</div></div>}
